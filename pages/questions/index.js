...
const QuestionManager = () => {
  ...
  useEffect(() => {
    if (dataQuestion?.length !== 0) {
      if (items.length == dataQuestion?.length) {
        setIsCheckAll(true);
      } else {
        setIsDisabled(false);
      }
  ...
  const handleDeleteMany = async () => {
    try {
      await questionService.deleteManyQuestion({
        ids: items,
      });
      getAllQuestion();
      setIsDisabled(true);
                    ...
                    <tbody>
                      {dataQuestion?.map((data) => (
                        <tr key={data._id}>
                          ...
                          <td>{data.question}</td>
                          <td className="name-post">{data.answer}</td>
                          ...
                          <td className="action">
                            <button
                              ...
                              onClick={() => {
                                Router.push(
                                  "/questions/create-question?id=" + data._id
                                );
                              }}
                            >
                              Xem
